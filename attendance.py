...
import holidays  # NEW: Estonia holiday calendar

###############################################################################
# Helper functions
###############################################################################

ee_holidays = holidays.EE()  # national & public holidays for all years
...
def working_days_in_month(year: int, month: int) -> int:
    ...
    return sum(
        ...
        if day
        and dow < 5  # Mon-Fri
        and datetime(year, month, day).date() not in ee_holidays
    )


def process_attendance(df: pd.DataFrame):
    """Compute attendance metrics. Expects columns:
    Employee name | Attendance date | Time in"""

    df = df.copy()
    df["Attendance date"] = pd.to_datetime(df["Attendance date"], format="%d.%m.%Y")
    df["Present"] = df["Time in"].notna()

    # Remove weekends & national holidays
    ...
    # --- Monthly working days (Mon-Fri minus holidays) ----------------------
    ...
    person_week = (
        df.groupby(["ISOWeek", "Employee name"])["Present"].sum()
        .reset_index(name="DaysInOffice")
    ...
    # Weekly
    team_week = (
        df.groupby("ISOWeek")["Present"].sum().reset_index(name="PersonDays")
        .merge(week_working_days, on="ISOWeek")
    )
    ...
    # Monthly
    team_month_persondays = df[month_mask]["Present"].sum()
    team_presence_month_pct = round(
        team_month_persondays / (working_days_month * team_size),
        2,
    )
